#blinds change, strategies stay, 5 players
#with these settings the occurence of specific achieved rounds during a 1000 trials from player 1 and player 2 are counted and printed
...
for i in range (1000):
    ...
    def strategy (name, color):
        if color[0] == color[1]:
            if name in excellentsuited or name in goodsuited or name in okaysuited:
                return True
        else:
            if name in excellent or name in good or name in okay:
                return True
    ...
    while stack > 2*smallblind and stack2 > 2*smallblind and rounds < 50:
        ...
        for i in range (0,2):
            ...
            karten.pop(n)
            n=randint(0,(48-5*i))
            hand4.append(karten[n])
            karten.pop(n)
            n=randint(0,(47-5*i))
            hand5.append(karten[n])
            karten.pop(n)
        ...
        for i in range (0,2):
            ...
            handname4.append(hand4[i]%13)
            handfarbe4.append(hand4[i]//13)
            handname5.append(hand5[i]%13)
            handfarbe5.append(hand5[i]//13)
        hand = []
        hand2 = []
        hand3 = []
        hand4 = []
        ...
        handname4.sort(reverse=True)
        handname5.sort(reverse=True)
        #blinds
        if rounds % 5 == 0: # every fith turn the player has to pay the big bilnd
            stack -= 2* smallblind
        elif rounds % 5 == 1:
            stack2 -= 2* smallblind
            stack -= smallblind
        ...
        if strategy2(handname3, handfarbe3):
            ...
            handname3.sort()
            validations.append(validation(handname3, handfarbe3))
        if strategy2(handname4, handfarbe4):
            hand4 += communitycards
            handname4 += communityname
            handfarbe4 += communitycolor
            handname4.sort()
            validations.append(validation(handname4, handfarbe4))
        if strategy3(handname5, handfarbe5):
            hand5 += communitycards
            handname5 += communityname
            handfarbe5 += communitycolor
            handname5.sort()
            validations.append(validation(handname5, handfarbe5))
        validations.sort(reverse=True)
        ...
        if strategy(handname,handfarbe):
            ...
            if strategy2(handname2, handfarbe2):
                ...
                pot += bet*(len(validations)-1)
                

                if validation1 > validation2 and validation1 > validations[0]:
                    stack += pot
                elif validation1 < validation2 and validation2 > validations[0]:
                    stack2 += pot
...
quantround =[]
for i in range (1,51):
    quantround.append(completedround2.count(i))
for i in range (50):
    print (quantround[i])
